  ...
  getStandardProficiencyList(duration): void {
    ...
    this.districtService.getDistrictProfile().subscribe((response) => {
      if (response && response.data) {
        this.districtService.getClassActivityReport(response.data.district_admin.id, duration).subscribe((res) => {
          if (res.data && res.data.rows) {
            _.map(res.data.rows, (item, index) => {
              this.districtService.getStandardProficiencyPercentage(duration, item.id).subscribe((res) => {
                if (res.data) {
