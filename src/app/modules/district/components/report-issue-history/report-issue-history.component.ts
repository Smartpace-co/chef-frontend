...
export class ReportIssueHistoryComponent implements OnInit {
  ...
  getAllReportHistoryDetails() {
    ...
    this.districtService.getAllReportHistories(this.activateUserData.id).subscribe((response) => {
  ...
  getReportReplies(reportReplies) {
   this.router.navigate(['/district/report-history-inner'], { queryParams: { id: reportReplies.id } });
