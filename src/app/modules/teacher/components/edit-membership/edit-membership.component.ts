...
export class EditMemberShipComponent implements OnInit {
  ...
  slideConfig: any = {
    ...
    constructor(private toast: ToasterService, private teacherService: TeacherService, private modalService: NgbModal, private router: Router) { }
  planList = [];
  isLoad = true;
  currentLoggedUser;
  selectedPlan;
  ...
  getPackageList(): void {
    ...
    var date=new Date();
        ...
        if (this.currentLoggedUser.token) {
          this.teacherService.getPackageListForEdit(isPrivate, this.currentLoggedUser.token, this.packageId,customElements,this.currentLoggedUser.role.id).subscribe(
            (res) => {
              let notUndefined;
  ...
  onSelectedPackage(): void {
    this.teacherService.getTeacherProfile(this.currentLoggedUser.id).subscribe(res=>{
      if(res && res.data)
      {
        this.teacherService.getCountStudents(this.currentLoggedUser.id,this.currentLoggedUser.role.id,this.selectedPlan.id).subscribe(res1=>{
          ...
          if( this.classCount>this.selectedPackageMaxUserCount)
          {
            ...
            this.closeModal1=this.modalService.open(this.proceedDeleteClassModal, { ariaLabelledBy: 'modal-basic-title', centered: true, windowClass: 'proceed-delete-class-modal' })
            }
          else
          {
    ...
    this.teacherService.createSubscriptionPackage(submission).subscribe(
      (res) => {
        if (res && res.data) {
       ...
       sessionStorage.setItem('currentUser', JSON.stringify(this.currentLoggedUser));
          ...
          this.teacherService.createStripePaymentSession(stripeData,this.currentLoggedUser.token).subscribe((dt) => {
