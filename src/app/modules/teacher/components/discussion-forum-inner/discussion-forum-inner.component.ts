...
export class DiscussionForumInnerComponent implements OnInit {
  ...
  constructor(
    private modalService: NgbModal, private actRoute: ActivatedRoute, private teacherService: TeacherService, private toast: ToasterService
  ) {
  ...
  openReply(replyId,replyUserId,comment) {
    ...
    {
      this.teacherService.deleteReply(replyId).subscribe(
        (response) => {
  ...
  getDiscussionForumInfoById() {
    this.teacherService.getDiscussionForumInfoById(this.discussionForumId).subscribe(
      (response) => {
        if (response && response.data) {
  ...
  getComments() {
    this.teacherService.getComments(this.discussionForumId).subscribe(
      (response) => {
        if (response && response.data) {
  ...
  getReplies(comment) {
    this.replyArray=[]
      this.teacherService.getReplies(comment.id).subscribe(
        (response) => {
          if (response && response.data) {
    ...
    if (this.deleteCommentForm.invalid) {
      this.toast.showToast('Please enter information for required fields', '', 'error');
        ...
        if (!_.isEmpty(this.deleteCommentForm.value.delete) && this.deleteCommentForm.value.delete === "DELETE") {
          this.teacherService.deleteComment(this.deleteCommentId).subscribe(
            (response) => {
              if (response && response.data) {
                this.closeOpenModal();
                this.getComments();
  ...
  submitComment(): void {
    ...
    if (this.commentForm.invalid) {
      ...
      this.teacherService.getTeacherProfile(this.activateUserData.id).subscribe(
        (response) => {
          if (response && response.data) {
            ...
            this.teacherService.addComment(submission).subscribe(
              (data) => {
                if (data) {
  ...
  addVote(vote: boolean): void {
    ...
    this.teacherService.updateVote(voteSubmission).subscribe(res => {
      if(res)
        this.getDiscussionForumInfoById();
    })
    let submission = {
    ...
    this.teacherService.addVote(this.discussionForumId, submission).subscribe(
      (data) => {
        this.getDiscussionForumInfoById();
      });
  }
  ...
  getVote(discussionForumId): void {
    this.teacherService.getVote(discussionForumId).subscribe(res => {
      if (res && res.data) {
      ...
      this.teacherService.getTeacherProfile(this.activateUserData.id).subscribe(
        (response) => {
          if (response && response.data) {
            ...
            this.teacherService.addReply(submission).subscribe(
              (data) => {
                if (data) {
