...
export class AdminContentReportComponent implements OnInit {
  ...
  getAllTopRatedLesson(duration) {
    this.schoolService.getAllTopRatedLesson(duration).subscribe((response) => {
      if (response && response.data && response.data) {
        this.adminContentList = _.map(response.data, (item) => {
          ...
          this.schoolService.getNumberOfTimesLessonAssigned(item.recipe.id).subscribe((res) => {
            obj.time = res['data'].count;
          });

          if (item.rating == 1) obj.rating = [true, false, false, false, false];
          else if (item.rating == 2) obj.rating = [true, true, false, false, false];
          else if (item.rating == 3) obj.rating = [true, true, true, false, false];
          else if (item.rating == 4) obj.rating = [true, true, true, true, false];
          else if (item.rating == 5) obj.rating = [true, true, true, true, true];
